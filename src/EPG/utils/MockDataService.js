...
export default class MockDataService {
  ...
  static createEvents(epgChannel, nowMillis) {
    ...
    while (currentTime <= epgEnd) {
      ...
      let epgEvent = new EPGEvent(
        currentTime,
        eventEnd,
        MockDataService.availableEventTitles[Math.floor(Math.random() * 6 + 0)],
      );
  ...
  static getEventEnd(eventStartMillis) {
    let length =
      MockDataService.availableEventLength[Math.floor(Math.random() * 4 + 0)];
