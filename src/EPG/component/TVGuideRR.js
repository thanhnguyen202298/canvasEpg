...
const TVGuide1 = ({ epgData, programs=[] }) => {
  ...
  const getScrollX = useCallback((neglect = true) => {
    if (neglect) {
      return 0;
    }
  ...
  const getScrollY = useCallback((neglect = true) => {
    if (neglect) {
      return 0;
    }
  ...
  const drawTimebarDayIndicator = useCallback(
    ...
    [getWidth, isRTL, getScrollY, getScrollX],
  );
  ...
  const drawTimebarBottomStroke = useCallback(
    ...
    [getScrollX, getScrollY, getWidth],
  );
  ...
  const drawTimeLine = useCallback(
    ...
    [getXFrom, getScrollY, getHeight, shouldDrawTimeLine],
  );
  ...
  const drawEvent = useCallback(
    (canvas, channelPosition, event, drawingRect) => {
      ...
      const end = moment(date2).format('HH:MM');
      canvas.fillText(
        `Start: ${start}`,
        drawingRect.left,
  ...
  const getDrawingRectForChannelImage = useCallback((drawingRect, image) => {
    drawingRect.left += stateCanvas.mChannelLayoutPadding;
